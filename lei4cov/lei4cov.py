...
class Lei4Cov(object):
    def __init__(self, n_input, d_model, batch_size, maxseq_len, d_ff, num_blocks, num_heads, dropout_rate,
                 log_eps=1e-8,
    ...
    def partial_fit(self, x=None, i_vec=None, j_vec=None, vi=None, vj=None):
        cost, opt, wi_emb = self.sess.run((self.cost, self.optimizer, self.wi_emb),
                                          feed_dict={self.idx: x, self.i_vec: i_vec, self.j_vec: j_vec, self.vi: vi,
                                                     self.vj: vj})
        return cost
